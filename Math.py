...
def AccuratePlace(card_img_hsv, limit1, limit2, color):
...
def Color(card_imgs):
    ...
    for card_index, card_img in enumerate(card_imgs):
        ...
        if limit1 == 0:
            continue
        xl, xr, yh, yl = AccuratePlace(card_img_hsv, limit1, limit2, color)
        if yl == yh and xl == xr:
            continue
        need_accurate = False
        if yl >= yh:
